 ...
 *
 -->
<template>
  <div class="tiny-ip-segmentation-box">
    <ul
      :class="{
        active: state.active,
        disabled: state.disabled,
        'tiny-ip-segmentation-box__input': true
      }"
      ...
      <li v-for="(item, index) of state.address" :key="index" :style="state.lineHeightStyle">
        ...
        <template v-if="index < state.address.length - 1">
          <slot :slot-scope="{ state, index, item }">
            <span
              v-if="type === 'IPv6' && delimiter === 'icon-dot-ipv4'"
              class="tiny-ip-segmentation-box__input__ipv6-delimiter"
              >:</span
            >
...
<script lang="ts">
import { renderless, api } from '@opentiny/vue-renderless/ip-segmentation-box/vue'
